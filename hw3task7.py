...
def preprocess_training_data_dt(data):
    ...
    X = X.drop(X.columns[[0,2,7,9]], axis=1)  
...
def build_DT(X_train, Y_train, X_test):
    ...
    dt = tree.DecisionTreeClassifier(splitter="best", max_depth=4, max_leaf_nodes=(7)) #29 ~82% accuracywas best
...
scores = cross_validation(linearSVC, X_train, Y_train)
print("Linear SVM - 5-Fold Cross Val Scores:")
print("%0.2f accuracy with a standard deviation of %0.2f \n" % (scores.mean(), scores.std()))


quadraticSVC = svm.SVC(kernel = 'poly', degree=2)
quadraticSVC.fit(X_train, Y_train)

scores2 = cross_validation(quadraticSVC, X_train, Y_train)
print("Quadratic SVM - 5-Fold Cross Val Scores:")
print("%0.2f accuracy with a standard deviation of %0.2f \n" % (scores2.mean(), scores2.std()))

rbfSVC = svm.SVC(kernel = 'rbf')
rbfSVC.fit(X_train, Y_train)

scores3 = cross_validation(rbfSVC, X_train, Y_train)
print("RBF SVM - 5-Fold Cross Val Scores:")
